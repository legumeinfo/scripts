...
This script is adapted from original Bio::GMOD::DB::Adapter.pm
This module should only be used for specifying feature type --src_type and for loading iprscan GFF data.
...
use constant SEARCH_NAME =>
               "SELECT feature_id FROM feature WHERE name=? and type_id=?"; #peu added type_id
use constant COUNT_NAME =>
               "SELECT COUNT(*) FROM feature WHERE name=? and type_id=?"; #peu added type_id
...
sub new {
    ...
    $self->src_type(        $arg{src_type}        );  #peu added
...
#peu sub created for src feature type specifying at cmd line
sub src_type {
	my $self = shift;
	my $s_type = shift if defined(@_);
my $sth = $self->dbh->prepare("SELECT cvterm_id FROM cvterm WHERE name = '$s_type'");
$sth->execute;
my $src_type = $sth->fetchrow_array;

#	return $self->{'src_type'} = $src_type if defined($src_type);
#	return $self->{'src_type'};
return $src_type if defined($src_type);
}
...
sub handle_target {
    ...
    my $organism_id; #==peu added
      my $featuretype = $feature->type->name;	
	
        my $type =  $self->cache('type',$featuretype);
	my $sql = $self->dbh->prepare("SELECT organism_id FROM organism where common_name='consensus'"); # use 'consensus' organism for unique polypeptide_domain features.
	$sql->execute();
        my ($org_id) = $sql->fetchrow_array;

	if ($featuretype eq 'protein_hmm_match') # do not use $type here as I think its for type_id not name of type 
	{
	 $organism_id = $org_id;
	}
	else
	...
	#revisit#2: change feature type to may be 'polypeptide_domain' manually outside loader for all abstract HMM representations of domains #adf thoughts# peu

   #please note, each gff line with Name and ID attribute get created as a seperate feature by loader script using some other sub and is seperate from Target attri feature that this sub creates#it does not relate to this sub#peu
  
    my @targets = $feature->annotation->get_Annotations('Target');
    my $rank = 1;
    open FTEST, ">>/home/peu/test_ctc.txt";#CTC CHANGED
    my $count_ctc = 0;
    print FTEST scalar(@targets)."\n";
    foreach my $target (@targets) {
      ...
      my $tsource   = ref($feature->source) 
        ...
        # I think this creates a feature_synonym in chado if feature name is repeated which will have same synonym_id for features with such same name but diff uniquename & feature_ids #peu
      ...
      #check for an existing feature with the Target's uniquename 
       #peu note: here gff rows with NO target does not come to handle_target subroutine but go to other sub and so they use the standard organim_id from cmdline
    #  my $real_target_id = $self->modified_uniquename(orig_id => $target_id, organism_id => $organism_id) ? #here the gff row with NO 'Target' will not be checked and will be simply inserted by above $self->synonym
     #                      $self->modified_uniquename(orig_id => $target_id, organism_id => $organism_id)
      #                   : $target_id; #this gives uniquenema based or target id and org, but this same uniquename can have multiple entries with diff org so below add org constraint also on uniquename cache
    #went into if for those that have 'protein_hmm_match' as type
	  if ($self->uniquename_cache(validate=>1,uniquename=>$target_id,organism_id=>$organism_id)){#this if-condition checks for presence of feature_id with 'Target id' and the organism chosen above in database 
	          $self->print_floc(
                            $self->nextfeatureloc,
                            $self->nextfeature,
                            $self->uniquename_cache(validate=>1,uniquename=>$target_id,organism_id=>$organism_id), #peu edited, this returns feature_id present in chado
                            $tstart, $tend, $tstrand, '\N',$rank,'0'
            );
		

	#	$self->print_f($self->nextfeature, $self->organism_id, $name,$tuniquename , $type, '\N','\N'); #peu added for feature insert
	  print FTEST "I AM IN AN IF FOR EXISTENCE count $count_ctc: I AM and organism: $organism_id and $target_id and my name is: ".$self->uniquename_cache(validate=>1,uniquename=>$target_id,organism_id=>$organism_id)."!\n";

      }
      else {
	  print FTEST "I AM IN AN ELSE FOR EXISTENCE count $count_ctc: I AM $target_id  and organism: $organism_id!\n";
          #$self->create_target_feature($name,$featuretype,$uniquename,$real_target_id,$type,$tstart,$tend,$tstrand,$rank); commented out by peu
                 
		$self->create_target_feature($name,$featuretype,$organism_id,$uniquename,$target_id,$type,$tstart,$tend,$tstrand,$rank); #peu added
     ...
     my $featuretype = $feature->type->name;  #commented out by peu

      my $type = $self->cache('type',$featuretype);  #commented out by peu
...
#       }
    ...
    close (FTEST); #CTC CHANGED
}

sub create_target_feature {
    my $self = shift;
    #my ($name,$featuretype,$uniquename,$target_id,$type,$tstart,$tend,$tstrand,$rank) = @_;  #commented out by peu
   
    my ($name,$featuretype,$organism_id,$uniquename,$target_id,$type,$tstart,$tend,$tstrand,$rank) = @_;   #peu
   ...
   # $self->print_f($self->nextfeature, $self->organism_id(), $name,$tuniquename , $type, '\N','\N');   ##commented out by peu
# $self->print_f($self->nextfeature, $organism_id , $name,$tuniquename , $type, '\N','\N'); #added by peu #probaly for feature



my $sth = $self->dbh->prepare("SELECT feature_id FROM feature
                  WHERE uniquename = ? and type_id = ? and organism_id = ?");
$sth->execute($tuniquename, $type, $organism_id);
my ($check) = $sth->fetchrow_array;

if (!$check) {

	$self->print_f($self->nextfeature, $organism_id, $name,$tuniquename , $type, '\N','\N');
}


    $self->print_floc(            #peu commented out #probably for featureloc
           $self->nextfeatureloc,
           ($self->nextfeature)-1,
          $self->nextfeature,
          $tstart,
...
#open FOUT_TEST, ">/home/peu/test_out.txt";  #CTC CHANGED
...
sub src_second_chance {
    my $self = shift;
    my ($feature, $src_type) = @_;

    my $organism_id = $self->organism_id;
     
    #my $src_type = $feature->type_id;
my $sth = $self->dbh->prepare("SELECT cvterm_id FROM cvterm WHERE name = '$src_type'");
$sth->execute;
my $type = $sth->fetchrow_array;	
    ...
    if($feature->seq_id eq '.'){
      ...
      unless ($temp_f_id) {
        $self->{queries}{count_name}->execute($src_uniquename, $type); #peu
       my ($n_rows) = $self->{queries}{count_name}->fetchrow_array;
       if (1 < $n_rows) {
         $self->throw( "more that one source for ".$src_uniquename );
       } elsif ( 1==$n_rows) {
        $self->{queries}{search_name}->execute($src_uniquename, $type); #peu
    ...
    if($feature->seq_id eq '.'){
     ...
     #      : $feature->seq_id;
      ...
      my ($temp_f_id)= $self->uniquename_cache(
                                        ...
                                        type_id => $type,       #added by peu
					organism_id => $organism_id #peu
					);
     # $self->cache('feature',$src_uniquename,$temp_f_id);
$self->cache('feature',$temp_f_id);
     # unless ($temp_f_id) {
...
#  $self->{queries}{count_name}->execute($src_uniquename);
     if ($temp_f_id){
       ...
       my ($n_rows) = $self->{queries}{count_name}->fetchrow_array;
        ...
        } elsif ( 1==$n_rows) { print "there is one entry \n";
...
my $sth = $self->dbh->prepare("SELECT cvterm_id FROM cvterm WHERE name = '$src_type'");
$sth->execute;
my $new_type = $sth->fetchrow_array;
...
my $organism_id = $self->organism_id;
  ...
  my ($src,$seqlen);
    if ( defined(($feature->annotation->get_Annotations('ID'))[0])
        ...
        if ( defined($new_type)) #use new_type and not src_type #correction#1
        {
 ...
 my $src_uniquename
     ...
     $src = $self->uniquename_cache(
                                        ...
                                        organism_id => $organism_id,
                                        uniquename  => $src_uniquename,
                                        type_id => $new_type,  #peu-adf added 
                                        );
 ...
 #        print "my command src is $src \n";
     ...
     $seqlen = $feature->end - $feature->start +1;
      ...
      $src = $self->uniquename_cache(
					...
					organism_id => $organism_id,
					uniquename  => $src_uniquename,
					type_id => $new_type,  #general case when src_type not provided should not need type here? peu
					);
