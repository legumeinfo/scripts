...
use lib '/opt/chado/1.23+/lib';

use strict;
use warnings;
#use lib '/Users/cain/cvs_stuff/schema/trunk/chado/lib';
#This is a modified gmod_bulk_load_gff3.pl script for clean iprscan GFF data loading, where we added --src_type option to specify feature type #peu 2015
...
=head1 OPTIONS
 ...
 --allow_external_parent 
  ...
  --src_type        Specify srcfeature type eg. polypeptide, chromosome etc
...
my ($ORGANISM, $GFFFILE,$FASTAFILE,$DBPROFILE, $DBNAME, $DBUSER, $DBPASS,$DBHOST, $DBPORT, 
    ...
    $PRIVATE_SCHEMA, $USE_PUBLIC_CV, $NO_SKIP_VACUUM, $BEGIN_SQL, $END_SQL, $REMOVE_LOCK,
    $ALLOW_EXTERNAL_PARENT,$SRC_TYPE,);
...
if ($SRC_TYPE) {  #peu
 $chado->src_type($SRC_TYPE) or die "$SRC_TYPE type not found in the database";
}
...
;
  ...
  my ($src, $seqlen) = $chado->get_src_seqlen($feature, $SRC_TYPE); 

  if(!$src){
    $src = $chado->src_second_chance($feature, $SRC_TYPE);
  }
